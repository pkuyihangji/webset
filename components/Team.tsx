...
export default function Team() {
  return (
    <section id="team" className="py-32 px-8 max-w-7xl mx-auto">
      <p className="text-amber-400 text-xs tracking-[0.3em] uppercase mb-4">Team</p>
      ...
      <div className="grid md:grid-cols-2 gap-px bg-white/5">
        {members.map((m) => (
          <div key={m.name} className="bg-[#080c14] p-10 hover:bg-white/[0.02] transition-colors">
            <div className="w-10 h-10 rounded-full border border-amber-400/30 flex items-center justify-center text-amber-400 font-mono text-sm mb-8">
              {m.name[0]}
            </div>
            <h3 className="text-white font-medium text-lg mb-1">{m.name}</h3>
            <p className="text-amber-400/60 text-xs tracking-widest uppercase mb-4">{m.role}</p>
